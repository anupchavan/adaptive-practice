...
/**
 * Coerces stored filter rules (settings.filterRules from data.json) into a
 * valid FilterGroup.
 *
 * The stored shape is unchanged from earlier plugin releases: this engine
 * reads the same `{ type: "group", operator, conditions }` tree, and every
 * operator the old builder could emit is still evaluated the same way, so
 * existing rules keep their semantics. Only structurally malformed entries
 * (wrong `type`, non-string field/operator) are dropped; an invalid root
 * falls back to the default match-everything group.
 */
export function normalizeFilterRules(raw: unknown): FilterGroup {
	return (
		normalizeGroup(raw) ??
		(JSON.parse(JSON.stringify(DEFAULT_FILTER_RULES)) as FilterGroup)
	);
}

function normalizeGroup(raw: unknown): FilterGroup | null {
	if (!raw || typeof raw !== "object") return null;
	const record = raw as Record<string, unknown>;
	if (record["type"] !== "group") return null;
	const operator: FilterConjunction =
		record["operator"] === "OR" || record["operator"] === "NOR"
			? record["operator"]
			: "AND";
	const rawConditions = Array.isArray(record["conditions"]) ? record["conditions"] : [];
	const conditions: (Filter | FilterGroup)[] = [];
...
function normalizeFilter(raw: unknown): Filter | null {
	if (!raw || typeof raw !== "object") return null;
	const record = raw as Record<string, unknown>;
	if (record["type"] !== "filter") return null;
	if (typeof record["field"] !== "string" || typeof record["operator"] !== "string") {
		return null;
	}
	// The builder can emit numeric comparators ("=", "≥", ...) that sit outside
	// the FilterOperator union; they are preserved verbatim like before.
	const filter: Filter = {
		type: "filter",
		field: record["field"],
		operator: record["operator"] as FilterOperator,
	};
	if (typeof record["value"] === "string") filter.value = record["value"];
	return filter;
}
 ...
 * @returns True if the group's conditions are met, false otherwise
 */
...
function evaluateFilter(app: App, filter: Filter, file: TFile, frontmatter?: FrontMatterCache): boolean {
	...
	if (Array.isArray(targetValue)) {
		...
		switch (filter.operator) {
			...
			case "is":
			case "is not": {
				const match = targetArray.some((v) => String(v) === filterValue);
			...
			case "is not exactly": {
				...
				const targetStrings = targetArray.map((v) => String(v));
			...
			case "does not contain": {
				const match = targetArray.some((v) => String(v).includes(filterValue));
			...
			case "does not contain any of": {
				...
				const match = filterValues.some((filterVal) =>
					targetArray.some((v) => String(v).includes(filterVal))
				);
