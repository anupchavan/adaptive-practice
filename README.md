...
- Multi-provider BYOK support for Gemini, Anthropic, OpenAI, DeepSeek, Qwen, OpenRouter, local Ollama, Claude Code and Codex subscriptions (via the native engine), and custom OpenAI-compatible endpoints.
...
| Claude Code (subscription) | `sonnet` | Runs on your Claude subscription through the Whetstone native engine (desktop, CLI installed and signed in). Model aliases: `sonnet`, `opus`, `fable`. |
| Codex CLI (subscription) | `terra` | Runs on your ChatGPT subscription through the Whetstone native engine (desktop, CLI installed and signed in). Model aliases: `terra`, `sol`, `luna`. |
